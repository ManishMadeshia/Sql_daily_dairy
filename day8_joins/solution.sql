...
select p.productid,p.productname,p.price,os.orderid,os.quantity from products p left join orderdetails os on p.productid = os.productid
select * from products p left join orderdetails os on p.productid = os.productid where os.orderid is null
select e.employeeid, e.firstname,e.salary,o.orderid from employees e left join orders o on e.employeeid = o.employeeid 
select e.employeeid, e.firstname,e.salary,o.orderid from employees e left join orders o on e.employeeid = o.employeeid is null
select d.departmentname,count(e.employeeid) as emp_count from departments d left join employees e on d.departmentid = e.departmentid group by d.departmentname
select c.customername,count(o.orderid) as total_orders from customers c left join orders o on c.customerid = o.customerid group by c.customername
select p.productname,sum(os.quantity) as qty from products p left join orderdetails os on p.productid = os.productid group by p.productname
